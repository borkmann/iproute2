			  ...
			  struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (matches(*argv, "mode") == 0) {
			...
			addattr32(n, 1024, IFLA_META_MODE, mode);
...
static void meta_print_opt(struct link_util *lu, FILE *f, struct rtattr *tb[])
{
	if (!tb)
		return;
	if (tb[IFLA_META_MODE] &&
	    RTA_PAYLOAD(tb[IFLA_META_MODE]) == sizeof(__u32)) {
		__u32 mode = rta_getattr_u32(tb[IFLA_META_MODE]);
		const char *mode_str =
			mode == META_L2 ? "l2" :
			mode == META_L3 ? "l3" : "unknown";

		print_string(PRINT_ANY, "mode", " mode %s ", mode_str);
	}
	if (tb[IFLA_META_PRIMARY] &&
	    RTA_PAYLOAD(tb[IFLA_META_PRIMARY]) == sizeof(__u8)) {
		__u8 primary = rta_getattr_u8(tb[IFLA_META_PRIMARY]);
		const char *type_str = primary ? "primary" : "peer";

		print_string(PRINT_ANY, "type", " type %s ", type_str);
	}
	if (tb[IFLA_META_POLICY] &&
	    RTA_PAYLOAD(tb[IFLA_META_POLICY]) == sizeof(__u32)) {
		__u32 policy = rta_getattr_u32(tb[IFLA_META_POLICY]);
		const char *policy_str =
			policy == META_PASS ? "forward" :
			policy == META_DROP ? "blackhole" : "unknown";

		print_string(PRINT_ANY, "policy", " policy %s ", policy_str);
	}
}

static void meta_print_help(struct link_util *lu,
...
struct link_util meta_link_util = {
	.id		= "meta",
	.maxattr	= IFLA_META_MAX,
	.parse_opt	= meta_parse_opt,
	.print_opt	= meta_print_opt,
